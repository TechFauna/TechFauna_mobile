    ...
    try {
      ...
      switch (formState.type) {
        case 'area': {
          ...
          if (!payload.name) {
            Alert.alert('Campo obrigatório', 'Informe o nome da área.');
            return;
          }
        ...
        case 'enclosure': {
          if (!selectedArea) {
            Alert.alert('Selecione uma área', 'Escolha em qual área ficará o recinto.');
            return;
          }
        ...
        case 'species': {
          ...
          if (!payload.common_name) {
            Alert.alert('Campo obrigatório', 'Informe o nome comum da espécie.');
            return;
          }
        ...
        case 'animal': {
          if (!selectedSpecies) {
            Alert.alert('Selecione a espécie', 'Escolha a espécie do animal.');
            return;
          }
      ...
      Alert.alert('Algo deu errado', error?.message || 'NA�o foi possível concluir a ação.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <ScrollView contentContainerStyle={styles.modalContent}>
            ...
            {formState.type === 'enclosure' && (
              <>
                ...
                <TextInput
                  style={styles.input}
                  placeholder="Tipo de ambiente (ex: aquático, savana)"
          ...
          >
            {submitting ? (
              ...
              <Text style={styles.primaryButtonLabel}>
                {formState.data?.id ? 'Salvar alterações' : 'Cadastrar'}
              </Text>
            )}
          </TouchableOpacity>
        </View>
...
const MoveAnimalModal = ({ moveState, setMoveState, enclosures, onRefresh }) => {
  ...
  const handleConfirm = async () => {
    ...
    if (!moveState.animal?.id) {
      Alert.alert('Animal inválido', 'Nãoo foi possível identificar o animal.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Movimentação não concluída', error?.message || 'Tente novamente mais tarde.');
    } finally {
      setSubmitting(false);
    }
  };
